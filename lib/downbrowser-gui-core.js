...
const { runFfmpegRemux } = require('./remux-helper');
...
class DownBrowserGuiCore extends EventEmitter {
  constructor(options = {}) {
    ...
    this.persistencePath = path.join(this.options.outputDir, '.downbrowser-gui-state.json');
  }

  addHistoryEntry(entry) {
    this.recordingHistory.unshift({
      id: entry.id || Date.now() + Math.random(),
      ...entry,
    });
    if (this.recordingHistory.length > 100) {
      this.recordingHistory.length = 100;
    }
  ...
  async loadPersistedState() {
    try {
      ...
      this.recordingQueue = Array.isArray(parsed.queue) ? parsed.queue : [];
      this.recordingHistory = Array.isArray(parsed.history) ? parsed.history : [];
      if (typeof parsed.autoRemux === 'boolean') {
        this.options.autoRemux = parsed.autoRemux;
      }
    } catch (error) {
      this.log('error', `Failed to restore GUI state: ${error.message}`);
    }
  }
  ...
  async persistState() {
    try {
      ...
      await fs.promises.writeFile(this.persistencePath, JSON.stringify({
        autoRemux: Boolean(this.options.autoRemux),
        queue: this.recordingQueue,
        history: this.recordingHistory,
      }, null, 2), 'utf8');
    } catch (error) {
      this.log('error', `Failed to persist GUI state: ${error.message}`);
    }
  }
  ...
  async getState() {
    ...
    return {
      ...
      recording: this.recording ? {
        ...
        remux: this.recording.remux || null,
      } : null,
      queue: this.recordingQueue.map((job) => ({
        id: job.id,
        tabId: job.tabId,
        ...
        remuxPath: job.remuxPath || null,
        error: job.error || null,
      })),
      history: this.recordingHistory.map((entry) => ({ ...entry })),
      logs: this.logs.slice(-120),
    };
  }

  async enqueueRecording(sourceIndex = 0, name = '') {
    ...
    this.recordingQueue.push(job);
    this.log('info', `Queued recording for tab ${tab.id}, source ${sourceIndex}`);
    await this.persistState();
    if (!this.recording || this.recording.result || this.recording.error) {
      await this.startNextQueuedRecording();
    }
    return this.getState();
  }

  async cancelQueueJob(jobId) {
    const job = this.recordingQueue.find((item) => item.id === jobId);
    if (!job) {
      throw new Error('Queue job not found');
    }
    if (job.status === 'running' || job.status === 'starting') {
      throw new Error('Cannot cancel a running queue job. Stop the active recording instead.');
    }
    ...
    this.recordingQueue = this.recordingQueue.filter((item) => item.id !== jobId);
    this.log('info', `Cancelled queue job ${String(jobId).slice(-6)}`);
    await this.persistState();
    return this.getState();
  }

  async removeQueueJob(jobId) {
    const before = this.recordingQueue.length;
    this.recordingQueue = this.recordingQueue.filter((item) => item.id !== jobId);
    if (this.recordingQueue.length === before) {
      throw new Error('Queue job not found');
    }
    this.log('info', `Removed queue job ${String(jobId).slice(-6)}`);
    await this.persistState();
    return this.getState();
  }

  async retryQueueJob(jobId) {
    const job = this.recordingQueue.find((item) => item.id === jobId);
    if (!job) {
      throw new Error('Queue job not found');
    }
    job.status = 'queued';
    job.error = null;
    this.log('info', `Retry queued for job ${String(jobId).slice(-6)}`);
    await this.persistState();
    if (!this.recording || this.recording.result || this.recording.error) {
      await this.startNextQueuedRecording();
    }
  ...
  async startNextQueuedRecording() {
    const nextJob = this.recordingQueue.find((job) => job.status === 'queued');
    if (!nextJob) {
      return;
    }
    ...
    try {
      await this.useTab(nextJob.tabId);
      await this.startRecording(nextJob.sourceIndex, nextJob.name, nextJob.id);
    } catch (error) {
      nextJob.status = 'failed';
      nextJob.error = error.message;
      this.log('error', `Queued recording failed to start: ${error.message}`);
  ...
  async startRecording(sourceIndex = 0, name = '', queueJobId = null) {
    if (this.recording && !this.recording.result && !this.recording.error) {
    ...
    const queueJob = queueJobId ? this.recordingQueue.find((job) => job.id === queueJobId) : null;
    if (queueJob) {
    ...
    this.recording.promise = downloadFromManifest(
      ...
      if (queueJob) {
        ...
        queueJob.outputPath = result.combinedOutputPath;
      }
      if (this.options.autoRemux) {
        this.recording.remux = {
          status: 'running',
          format: 'mp4',
        };
        this.scheduleStateEmit();
        try {
          const remuxPath = await runFfmpegRemux({
            input: result.combinedOutputPath,
            format: 'mp4',
            overwrite: true,
            ffmpeg: this.options.ffmpegPath || 'ffmpeg',
            stdio: 'pipe',
          });
          this.recording.remux = {
            status: 'completed',
          ...
          if (queueJob) {
            queueJob.remuxPath = remuxPath;
          }
          this.log('info', `Auto-remux complete: ${remuxPath}`);
        } catch (remuxError) {
          this.recording.remux = {
            status: 'failed',
            format: 'mp4',
          ...
          this.log('error', `Auto-remux failed: ${remuxError.message}`);
        }
      }
      this.addHistoryEntry({
        tabId: tab.id,
        ...
        name: baseName,
        status: 'completed',
        createdAt: new Date(this.recording.startedAt).toISOString(),
        completedAt: new Date().toISOString(),
        outputPath: result.combinedOutputPath,
        remuxPath: this.recording.remux?.outputPath || null,
      });
      await this.persistState();
  ...
  setAutoRemux(enabled) {
    this.options.autoRemux = Boolean(enabled);
    this.log('info', `Auto-remux ${this.options.autoRemux ? 'enabled' : 'disabled'}`);
    this.scheduleStateEmit();
    this.persistState().catch(() => {});
  }
